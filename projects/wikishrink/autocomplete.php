...
$temp = array();
	...
	$fetch = 'http://en.wikipedia.org/w/api.php?action=query&list=allpages&apfrom=' . $searchquery . '&aplimit=20&apfilterredir=all&format=xml';
	$result = httpGet($fetch); 
	$data = simplexml_load_string($result);
	
	foreach($data->query->allpages->p as $res) {
	
		$temp[] = array(
				"id" => (string)$res['title'][0], 
				"label" => (string)$res['title'][0],
				"value" => (string)$res['title'][0]
			);
			
				//print_r((string)$res['title'][0]);
